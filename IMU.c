void computeIMU () {
  ...
  //gyro only: the delay to read 2 consecutive values can be reduced to only 0.65ms
    ...
    for (axis = 0; axis < 3; axis++) {
      ...
      // empirical, we take a weighted value of the current and the previous values
	  // note: the result of gyroADCinter may overflow!!!   dongshan
...
// this one is also used for ALT HOLD calculation, should not be changed
  ...
  #define ACC_LPF_FACTOR 4 // that means a LPF of 16 // accelerator low pass filter factor
#endif

/* Set the Gyro Weight for Gyro/Acc complementary filter
   Increasing this value would reduce and delay Acc influence on the output of the filter*/
#ifndef GYR_CMPF_FACTOR
  #define GYR_CMPF_FACTOR 600	// gyroscope and accelerater complementary fileter factor weight
#endif

/* Set the Gyro Weight for Gyro/Magnetometer complementary filter
   Increasing this value would reduce and delay Magnetometer influence on the output of the filter*/
#define GYR_CMPFM_FACTOR 250 // gyroscope and magnetometer complementary fileter factor weight
...
// EstG.V is the gyroscope vector 
static t_fp_vector EstG;	// the float point one
static t_int32_t_vector EstG32;	// the int one
#if MAG
  static t_int32_t_vector EstM32;	//estimated megnitude vector
...
void getEstimatedAttitude(){
  ...
  // Initialization
  for (axis = 0; axis < 3; axis++) {
  	// get delta angle during currentT-previousT
  ...
  // EstG: estimated gravity vector
  rotateV(&EstG.V,deltaGyroAngle);
  #if MAG
  //EstM: estimated magnetic vector
    rotateV(&EstM.V,deltaGyroAngle);
  #endif
  /* 
   * setting f.SMALL_ANGLES_25 depends on the magnitude of its three axis accSmooth[x] value respectively 
   * 
   */
  ...
  // If accel magnitude >1.15G or <0.85G and ACC vector outside of
  //the limit range => we neutralize the effect of accelerometers in the angle estimation.
  ...
  // here i think it uses the small angle approximation method to get the angle of ROLL and PITCH.
